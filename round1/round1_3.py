...
class Trader:
    ...
    def get_squid_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
      """Calculate fair value for Squid Ink using mean-reversion strategy with rolling window"""
      ...
      window_size = self.params[Product.SQUID]["window_size"]
      if len(self.price_history[Product.SQUID]) < window_size:
          return current_price  # Not enough data yet

      recent_prices = list(self.price_history[Product.SQUID])[-window_size:]
    ...
    def get_kelp_fair_value(self, order_depth: OrderDepth, traderObject) -> float:
      ...
      if len(self.price_history[Product.KELP]) < window_size:
          return current_price

      prices = list(self.price_history[Product.KELP])[-window_size:]
      x = np.arange(len(prices))
      slope = np.polyfit(x, prices, 1)[0]  

      if slope > 0:  
          return current_price * (1 + abs(slope)*10) 
      elif slope < 0:  
          return current_price * (1 - abs(slope)*10) 
      return current_price
