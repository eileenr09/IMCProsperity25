...
BSK_VOL_WINDOW = 20  # Number of ticks to track for dynamic threshold of BSK 1 and 2
SQUID_VOL_WINDOW = 30
SQUID_MA_WINDOW = 5
...
class Trader:
    ...
    def update_volatility(self, product: str, mid: float):
        window = SQUID_VOL_WINDOW if product == SQUID else BSK_VOL_WINDOW
    ...
    def take_orders(self, state, product, fair_price, vol, k = 1):
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        if RESIN in order_depths:
            ...
            fair_value = 10000
            k = 1
            take_orders = self.take_orders(state, RESIN, fair_value, 0, k)
        ...
        for product in [PICNIC_BASKET1, PICNIC_BASKET2]:
            ...
            vol = self.volatility[product]
            k = 1
            orders = self.take_orders(state, product, fair_price, vol, k)
